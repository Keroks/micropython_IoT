...
from mpu6050_regs import *
import cfilter
import gc

default_sample_rate = 0x20

default_calibration_numsamples = 400
default_calibration_accel_deadzone = 20
default_calibration_gyro_deadzone = 10
...
class MPU(object):
    def __init__(self, i2c, pin_led, rate=default_sample_rate,
                 address=MPU6050_DEFAULT_ADDRESS):
        self.bus = i2c
        self.pin_led = PWM(Pin(pin_led, mode=Pin.OUT))
        self.rate = rate
    ...
    def write_uword(self, reg, val):
        self.bus.writeto_mem(self.address, reg, pack('>H', val))

    def write_word(self, reg, val):
        self.bus.writeto_mem(self.address, reg, pack('>h', val))

    def read_uword(self, reg):
    ...
    def reset_offsets(self):
        self.write_word(MPU6050_RA_XA_OFFS_H, 0)
        self.write_word(MPU6050_RA_YA_OFFS_H, 0)
        self.write_word(MPU6050_RA_ZA_OFFS_H, 0)
        self.write_word(MPU6050_RA_XG_OFFS_USRH, 0)
        self.write_word(MPU6050_RA_YG_OFFS_USRH, 0)
        self.write_word(MPU6050_RA_ZG_OFFS_USRH, 0)
